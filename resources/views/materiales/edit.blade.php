...
@section('title', '- Editando material')

@section('content')
    <div class="container">
        <h1><i class="mdi mdi-plus" aria-hidden="true"></i> Editando material</h1>
        <form method="POST" action="{{ route('materiales.update', $material) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="id" class="form-control" value="{{ old('id', $material->id) }}" readonly>
                @error('id')
            ...
            <div class="form-group">
                ...
                <input type="text" name="codigo" class="form-control" value="{{ old('codigo', $material->codigo) }}">
                @error('codigo')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="descripcion" class="form-control" value="{{ old('descripcion', $material->descripcion) }}">
                @error('descripcion')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="id_tipo_uso">
                    @foreach($tiposUso as $tipoUso)
                        ...
                        {{ old('id_tipo_uso', $material->id_tipo_uso) == $tipoUso->id ? 'selected' : '' }}>{{ $tipoUso->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <!-- Tipo de vehículo -->
            <div class="form-group">
                ...
                <select class="form-control" name="id_tipo_vehiculo">
                    @foreach($tiposVehiculo as $tipoVehiculo)
                        ...
                        {{ old('id_tipo_vehiculo', $material->id_tipo_vehiculo) == $tipoVehiculo->id ? 'selected' : '' }}>{{ $tipoVehiculo->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <!-- Marca de vehículo -->
            <div class="form-group">
                ...
                <select class="form-control" name="id_marca" id="id_marca">
                    @foreach($marcasVehiculo as $marcaVehiculo)
                        ...
                        {{ old('id_marca', $material->id_marca) == $marcaVehiculo->id ? 'selected' : '' }}>{{ $marcaVehiculo->nombre }}</option>
                    @endforeach
                </select>
                @error('id_marca')
                    <div class="text-danger">{{ $message }}</div>
            ...
            <!-- Modelo vehículo -->
             <div class="form-group">
                ...
                @if(old('id_modelo'))
                    <span id="id_modelo_old" hidden>{{ old('id_modelo', $material->id_modelo) }}</span>
                @endif
                @error('id_modelo')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <input type="number" name="cantidad" class="form-control" value="{{ old('cantidad', $material->cantidad) }}">
                @error('cantidad')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="costo_unitario" class="form-control" value="{{ old('costo_unitario', $material->costo_unitario) }}">
                </div>
